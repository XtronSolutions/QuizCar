...
//				Console.Write("\n");
		...
		private static WPLightmapData PrepareLightmapData(string[] lightmapPaths, GameObject[] affectedObjects, float waterLevel, float wetnessHeight, float wetnessAmount) {
			...
			foreach (GameObject obj in affectedObjects) {
				...
				wpTerrainData.width = terrainData.heightmapResolution;
				wpTerrainData.height = terrainData.heightmapResolution;
				...
				float[,] tempHeightmap2d = terrainData.GetHeights(0, 0, terrainData.heightmapResolution, terrainData.heightmapResolution);
