...
public class SimpleCar : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        ...
        WheelRR.motorTorque = maxTorque * Input.GetAxis("Acceleration");
        WheelRL.motorTorque = maxTorque * Input.GetAxis("Acceleration");

        //changing car direction
        //Here we are changing the steer angle of the front tyres of the car so that we can change the car direction.
        WheelFL.steerAngle = 30 * (Input.GetAxis("Steering"));
        WheelFR.steerAngle = 30 * Input.GetAxis("Steering");
    }
    void Update()
    {
        HandBrake();
